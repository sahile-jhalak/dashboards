...
const CityChart = ({ height=320 })=>{
  const { filteredData } = useAppContext();
  const ref = useRef();
  const data = useMemo(()=>{
    const map = {};
    filteredData.forEach(d=>{ const k = d.city || "Unknown"; map[k] = (map[k]||0)+1; });
  ...
  useEffect(()=>{
    ...
    g.selectAll("rect").data(data).join("rect").attr("x",d=>x(d.key)).attr("y",d=>y(d.value)).attr("width",x.bandwidth()).attr("height",d=>innerH-y(d.value)).attr("fill","#0f766e");
    svg.append("text").attr("x", margin.left).attr("y", 12).text("Top Cities").style("font-weight","600").style("fill", "#ffffff").style("font-size", "16px").style("text-anchor", "start");
  }, [data]);
